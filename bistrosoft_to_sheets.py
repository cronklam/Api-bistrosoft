"""
bistrosoft_to_sheets.py  â€” Cronklam / L'Harmonie
==============================================================
Arquitectura:
    1. Consulta Bistrosoft API para UN SOLO DÃA (ayer por defecto).
       IMPORTANTE: el API de Bistrosoft devuelve todos los registros con la
       fecha del rango mÃ¡s reciente si se consulta un rango multi-dÃ­a.
       Por eso siempre usamos startDate = endDate = un dÃ­a especÃ­fico.
    2. Calcula el resumen diario en Python.
    3. PestaÃ±a "Resumen"        â†’ histÃ³rico acumulativo (merge inteligente).
    4. PestaÃ±a "Transacciones"  â†’ detalle del dÃ­a descargado.
    5. PestaÃ±a "Stock MÃ­nimo"   â†’ stock mÃ­nimo POR DÃA DE SEMANA = prom. qty Ãºltimos 2 mismos dÃ­as.
    6. PestaÃ±a "Promedio x DÃ­a" â†’ desglose diario: qty por artÃ­culo, local, fecha.

Sin Supabase. Sin VIEW. Sin dedup complejo.
Basado en el script original de Pomodoro Consulting (TomÃ¡s).
"""
...
GOOGLE_SHEET_TAB_PROMEDIO = "Stock MÃ­nimo"
GOOGLE_SHEET_TAB_DIARIO = "Promedio x DÃ­a"
GOOGLE_SHEET_TAB_OLD_VTA = "Vta Promedio x Dia"

# DÃ­a a consultar en modo normal.  None = ayer automÃ¡ticamente.
FECHA_ESPECIFICA = None

# â”€â”€ MODO BACKFILL â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
MODO_BACKFILL = False
BACKFILL_DESDE = "2026-03-01"
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

# â”€â”€ COLORES PARA FORMATO â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ Bistrosoft â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def obtener_token():
    ...
    if not token:
        raise Exception(f"No se encontrÃ³ token en respuesta: {data}")
    print(f"[{now()}] âœ… Token Bistrosoft obtenido")
    return token


def descargar_transacciones(token, fecha_desde, fecha_hasta):
    """Descarga TODAS las pÃ¡ginas del TransactionDetailReport para el rango dado."""
    url = f"{BISTRO_BASE_URL}/TransactionDetailReport"
    headers = {"Authorization": f"Bearer {token}"}
    todas, page = [], 0
    print(f"[{now()}] ðŸ“¥ Descargando Bistrosoft: {fecha_desde} â†’ {fecha_hasta}...")
    while True:
        ...
        todas.extend(items)
        print(f"[{now()}]   PÃ¡g {page + 1}/{total_pages} â€” {len(items)} registros")
        if page + 1 >= total_pages or not items:
            break
        page += 1
        time.sleep(5)
    print(f"[{now()}] âœ… {len(todas)} transacciones descargadas")
    return todas
...
def calcular_resumen(transacciones):
    ...
    print(f"[{now()}] ðŸ”„ Dedup tickets: {total_antes} registros â†’ {len(ticket_map)} Ãºnicos")
...
# â”€â”€ CÃ¡lculo de promedios por dÃ­a de semana â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _parse_monto(valor):
    """Parsea un valor monetario que puede venir como nÃºmero o string con formato argentino.
    ...
    if "," in s and "." in s:
        # Si la coma viene despuÃ©s del Ãºltimo punto â†’ formato argentino
        if s.rfind(",") > s.rfind("."):
            s = s.replace(".", "").replace(",", ".")
        else:
        ...
        # Solo coma â†’ puede ser decimal argentino "500,50"
        s = s.replace(",", ".")
    # Si quedan mÃºltiples puntos, eliminar todos menos el Ãºltimo (miles)
    parts = s.split(".")
    if len(parts) > 2:
...
# Orden para mostrar los dÃ­as en la planilla
_DIA_ORDEN = {d: i for i, d in enumerate(["Lunes", "Martes", "MiÃ©rcoles", "Jueves", "Viernes", "SÃ¡bado", "Domingo"])}


def calcular_stock_minimo(transacciones_ws):
    """
    Calcula stock mÃ­nimo POR DÃA DE SEMANA, por artÃ­culo, por local.
    FÃ³rmula:
      1. Agrupa transacciones por (local, producto, fecha).
      2. Suma qty por dÃ­a real.
      3. Para cada dÃ­a de semana (Lunes, Martes, â€¦), toma los Ãºltimos 2
         dÃ­as iguales (ej: los 2 Ãºltimos jueves).
      4. Stock mÃ­nimo = suma de esos 2 dÃ­as / 2 (promedio).
    """
    ...
    # Paso 2: agrupar por (shop, product, weekday) â†’ lista de (date, qty)
    # {(shop, product, weekday_num): [(date, qty), ...]}
    por_dia_semana = defaultdict(list)
    for (shop, product, fecha), qty in ventas_dia.items():
        wd = fecha.weekday()  # 0=Lun â€¦ 6=Dom
        por_dia_semana[(shop, product, wd)].append((fecha, qty))

    # Paso 3: para cada combo, tomar los Ãºltimos 2 dÃ­as y promediar
    resultado = []
    for (shop, product, wd), dias in por_dia_semana.items():
        # Ordenar por fecha descendente y tomar los 2 mÃ¡s recientes
...
# â”€â”€ Google Sheets â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
COLS_TRANS_DISPLAY = [
    ...
    "Cantidad", "Cliente", "Mozo", "Mesa", "CategorÃ­a",
]

COLS_STOCK_MIN = [
    "Local", "Producto", "DÃ­a",
    "Stock MÃ­nimo", "Fechas Usadas", "# DÃ­as",
]

COLS_DIARIO = [
    "Fecha", "DÃ­a Semana", "Local", "Producto", "Qty Total",
]


def calcular_desglose_diario(transacciones_ws):
    """
    Calcula el desglose diario de qty por artÃ­culo, local, fecha.
    Esto permite ver cuÃ¡nto se vendiÃ³ de cada artÃ­culo cada dÃ­a.
    """
    ventas_dia = defaultdict(float)
...
def _format_header(ws, num_cols, color_bg=None, color_fg=None):
    """Aplica formato al header (fila 1) de cualquier pestaÃ±a."""
    if color_bg is None:
...
def _format_number_cols(ws, col_indices, num_rows):
    """Formatea columnas numÃ©ricas con separador de miles."""
...
def actualizar_resumen_en_sheets(nuevo_resumen, sh):
    ...
    # Timestamp
    ws.update_cell(len(filas) + 2, 1, f"Ãšltima actualizaciÃ³n: {now()}")
    ws.format(f"A{len(filas) + 2}:H{len(filas) + 2}", {
        "textFormat": {"italic": True, "foregroundColor": {"red": 0.5, "green": 0.5, "blue": 0.5}},
    })

    print(f"[{now()}] âœ… Resumen actualizado â€” {len(todos)} filas histÃ³ricas "
...
def actualizar_transacciones_en_sheets(transacciones, sh):
    ...
    print(f"[{now()}] âœ… Transacciones actualizadas â€” {len(filas) - 1} filas")


def actualizar_promedios_en_sheets(sh):
    """
    Lee el histÃ³rico de Transacciones y genera la pestaÃ±a 'Stock MÃ­nimo'
    con el stock mÃ­nimo POR DÃA DE SEMANA, por artÃ­culo, por local.
    FÃ³rmula: promedio qty de los Ãºltimos 2 mismos dÃ­as de semana.
    """
    print(f"[{now()}] ðŸ“Š Calculando stock mÃ­nimo por dÃ­a de semana / artÃ­culo / local...")

    # â”€â”€ Leer historial de Transacciones â”€â”€
    try:
        ws_trans = sh.worksheet(GOOGLE_SHEET_TAB_TRANS)
        all_trans = ws_trans.get_all_records()
    except Exception as e:
        print(f"[{now()}] âš ï¸ No se pudo leer Transacciones: {e}")
        all_trans = []

    if not all_trans:
        print(f"[{now()}] âš ï¸ Sin datos para calcular stock mÃ­nimo")
        return
    ...
    filas = []
    # TÃ­tulo
    filas.append(["STOCK MÃNIMO POR DÃA DE SEMANA", "", "", "", "", ""])
    filas.append(["Promedio qty Ãºltimos 2 mismos dÃ­as (ej: 2 jueves)", "", "", "", "", ""])
    filas.append([""])
    ...
    # Formato numÃ©rico columnas C y D (Prom. Qty y Stock MÃ­nimo)
    data_end = len(filas)
    ...
    # Colorear filas alternadas â€” DESHABILITADO (excede rate limit)
    # Se omite zebra-striping para no superar 60 writes/min de Google Sheets API

    # Resaltar columna Stock MÃ­nimo con fondo amarillo claro
    if data_end > header_row:
        ws.format(f"D{header_row + 1}:D{data_end}", {
    ...
    # Timestamp
    ts_row = len(filas) + 2
    ws.update_cell(ts_row, 1, f"Ãšltima actualizaciÃ³n: {now()}")
    ws.format(f"A{ts_row}", {
        "textFormat": {"italic": True, "foregroundColor": {"red": 0.5, "green": 0.5, "blue": 0.5}},
    })

    print(f"[{now()}] âœ… Stock mÃ­nimo actualizado â€” {len(stock_data)} artÃ­culos")


def actualizar_diario_en_sheets(sh):
    """
    Lee las Transacciones y genera la pestaÃ±a 'Promedio x DÃ­a'
    con el desglose diario real: qty por artÃ­culo, local, fecha.
    """
    print(f"[{now()}] ðŸ“Š Calculando desglose diario por artÃ­culo / local / fecha...")
    ...
    if not all_trans:
        print(f"[{now()}] âš ï¸ Sin datos para desglose diario")
        return
    ...
    print(f"[{now()}] âœ… Desglose diario actualizado â€” {len(diario)} filas")


def limpiar_pestana_vieja(sh):
    """Limpia la pestaÃ±a vieja 'Vta Promedio x Dia' que es redundante."""
    try:
        ws_old = sh.worksheet(GOOGLE_SHEET_TAB_OLD_VTA)
        ws_old.clear()
        ws_old.update_cell(1, 1, "âš ï¸ Esta pestaÃ±a fue reemplazada por 'Stock MÃ­nimo' y 'Promedio x DÃ­a'")
        ws_old.update_cell(2, 1, f"Limpiada: {now()}")
        print(f"[{now()}] ðŸ§¹ PestaÃ±a vieja '{GOOGLE_SHEET_TAB_OLD_VTA}' limpiada")
    except Exception:
        pass  # No existe, OK
...
def main():
    ...
    print(f"  Bistrosoft â†’ Sheets ({modo}) | Cronklam | {now()}")
    print(f"{'='*60}\n")
    ...
    try:
        ...
        if MODO_BACKFILL:
            ...
            print(f"[{now()}] ðŸ—“ BACKFILL: {fecha_desde} â†’ {fecha_hasta} (puede tardar varios minutos)")
        elif FECHA_ESPECIFICA:
            fecha_desde = FECHA_ESPECIFICA
            print(f"[{now()}] ðŸ—“ Fecha especÃ­fica: {fecha_desde} â†’ {fecha_hasta}")
        else:
            fecha_desde = (hoy - timedelta(days=15)).strftime("%Y-%m-%d")
            print(f"[{now()}] ðŸ—“ Ventana 15 dÃ­as: {fecha_desde} â†’ {fecha_hasta}")

        # â”€â”€ Descargar transacciones â”€â”€
        # NOTA: la API de Bistrosoft NO soporta consultas de un solo dÃ­a
        # (startDate = endDate devuelve 0).  Se debe usar rango multi-dÃ­a.
        # La API puede devolver todos los registros con la fecha del
        # extremo mÃ¡s reciente; por eso confiamos en el campo "date" tal
        # cual viene.
        transacciones = descargar_transacciones(token, fecha_desde, fecha_hasta)
        print(f"[{now()}] âœ… Total descargado: {len(transacciones)} transacciones")

        if not transacciones:
            print(f"[{now()}] âš ï¸ 0 registros â€” Bistrosoft no tiene datos para esta ventana.")
            return

        # DistribuciÃ³n de fechas
        dist_fechas = Counter(t.get("date", "?") for t in transacciones)
        print(f"[{now()}] ðŸ”Í Fechas en respuesta: {len(dist_fechas)} dÃ­as distintos, "
        ...
        if not fechas_validas:
            print(f"[{now()}] âŒ No se encontraron fechas vÃ¡lidas.")
            return

        # En modo 15 dÃ­as y backfill procesamos TODOS los dÃ­as
        print(f"[{now()}] ðŸ“… Procesando {len(fechas_validas)} dÃ­a(s), "
              f"{len(transacciones)} registros totales")

        # Calcular resumen
        resumen = calcular_resumen(transacciones)
        print(f"\n[{now()}] ðŸ“Š Resumen ({len(resumen)} filas):")
        for r in resumen:
